...
public:
    ...
    Tensor transpose(const int dim1, const int dim2) const;
    Tensor squeeze(int dim = -1) const;
    Tensor unsqueeze(int dim) const;
    Tensor permute(const std::vector<size_t> &dim_order) const;

    // CNN核心运算
    Tensor conv2d(const Tensor &kernel, const std::vector<size_t> &stride = {1, 1}, const std::vector<size_t> &padding = {0, 0}) const;
    Tensor pad(const std::vector<size_t> &padding, const T &pad_value = T(0)) const;
    Tensor maxPool2d(const std::vector<size_t> &pool_size, const std::vector<size_t> &stride, const std::vector<size_t> &padding) const;
    Tensor avgPool2d(const std::vector<size_t> &pool_size, const std::vector<size_t> &stride, const std::vector<size_t> &padding) const;
    ...
    // 批量/维度运算
    Tensor sumDim(int dim, bool keepdim = false) const;
    Tensor meanDim(int dim, bool keepdim = false) const;
    Tensor expand(const std::vector<size_t> &new_shape) const;
    Tensor flatten(int start_dim = 0, int end_dim = -1) const;
    Tensor<size_t> argmax(int dim, bool keepdim = false) const;
};
